...
import { ALL_THEMES, CONSUMER_THEMES, VAR_MAP } from "@codellyson/justui";
...
function buildThemes() {
  ...
  return ALL_THEMES.map((t) => ({
    ...
    category: consumer.has(t.id) ? "consumer" : "dev",
    swatch: t.swatch,
    light: t.light,
    dark: t.dark,
  }));
...
export async function serializeRegistry(): Promise<string> {
  ...
  return [
    ...
    `window.JUSTUI_VAR_MAP = ${JSON.stringify(VAR_MAP, null, 2)};`,
    `window.JUSTUI_THEMES = ${JSON.stringify(buildThemes(), null, 2)};`,
    "",
  ].join("\n");
}
